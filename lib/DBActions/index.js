...
DBActions.prototype.remove = function (modelIdValue, next) {
    ...
    that.Model.remove(conditions, function(err, result){
        next(err, result);
        if (!err && result) {
            var evtObj = getModelEvent(that.modelName);
            _li("event: ", evtObj);
            evtObj && evtObj.onDelete && evtObj.onDelete(modelIdValue);
        }
...
DBActions.prototype.update = function (param, next) {
    ...
    Model.update(query, param, options, function (err, result) {
        next(err, result);
        if (!err && result) {
            var evtObj = getModelEvent(that.modelName);
            evtObj && evtObj.onUpdate && evtObj.onUpdate(modelIdValue);
        }
    });
};
    ...
    that.incrementCounter(function (err, counter) {
        ...
        model.save(function (err, result) {
            ...
            if (!err && result) {
                var evtObj = getModelEvent(that.modelName);
                _li("event: ", evtObj);
                evtObj && evtObj.onSave && evtObj.onSave(c);
