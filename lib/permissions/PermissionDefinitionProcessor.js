...
function insertSettingsPluginsPermission(app, next) {
    ...
    var pluginInstanceHandler = require(app.set('appPath') + "/lib/PluginInstanceHandler"),
        Roles  = require(app.set('appPath') + "/lib/permissions/Roles"),
...
var ProcessPermissions = exports.ProcessPermissions = function (app, next) {
    ...
    var mapFunction = function (pluginId, next) {
        ...
        if (FileUtil.exists(permissionDefinitionFile)) {
            ...
            try {
                ...
                dbActionInstance.multipleSave(arr, next);
            } catch (e) {
                Debug._l(e);
                next(e);
            }
    ...
    //finds existing permissions in SchemaPermissions schema, if present put in cache otherwise process permission definition file
    dbActionInstance.get('find', function (err, data) {
        if (err) {
            return next(err);
        }
