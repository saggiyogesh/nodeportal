...
function CacheItem(permissionSchemaKey, actionsValue, permissions) {
    if (!actionsValue) {
        if (!utils.contains(permissionSchemaKey, TRIPLE_UNDERSCORE)) {
            ...
            actionsValue = cache[key].actionsValue;
        }
    }
...
exports.store = function (obj) {
    ...
    cache[obj.permissionSchemaKey] = new CacheItem(obj.permissionSchemaKey, obj.actionsValue, obj.rolePermissions);
};
...
exports.remove = function (permissionSchemaKey, modelId) {
    ...
    delete cache[permissionSchemaKey];
};
...
/**
 * Stores permission cache by retrieving model from db using params: modelName & modelId.
 *
 ...
 * @param next {Function}
 */
exports.storeByModelId = function (db, permissionSchemaKey, modelName, modelId, next) {
    if (!permissionSchemaKey) {
        throw new Error("Permission Schema Key is not defined.");
    }
    ...
    function process() {
        ...
        if (!exports.getCacheItem(permissionSchemaKey)) {
            var dbAction = new DBActions.DBActions(db, {modelName: modelName, permissionSchemaKey: permissionSchemaKey});
            var modelIdKey = DBActions.getModelIdKey(modelName);

            var params = {};
            params[modelIdKey] = modelId;
            dbAction.get('findOne', params, function (err, model) {
                if (err) {
                    Debug._l(err);
                }
                else {
...
exports.getCacheItem = function (permissionSchemaKey, modelId) {
    ...
    return cache[permissionSchemaKey];
};
