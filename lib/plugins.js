...
function Plugin(opts, app) {
    ...
    this.path = path;
    this.name = opts.name;
    var pl = new require(FileUtil.realPath(app.set('appPath'), path, opts.controller));
    ...
    if (opts.config.defaultData) {
        this.defaultData = require(FileUtil.realPath(app.set('appPath'), path, "defaultData"));

        if (opts.config.defaultDataHandler) {
            this.defaultDataHandler = require(FileUtil.realPath(app.set('appPath'), path, "defaultData", opts.config.defaultDataHandler));
        }
    }
}
...
/*exports.init = function (app) {
 ...
 Debug._li(">> " , app.set("permissions"), true)
 if (err) throw err;
 if (res) {
