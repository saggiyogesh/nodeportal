 ...
 * @param layoutArgs {Array} array passed to retrieve the layout in generic way
 * @param next
 */
PageRenderer.prototype._init = function (layoutArgs, next) {
    ...
    async.parallel([
        function (n) {
            that.getService(THEME_SCHEMA).findById(page.themeId, n);
        },
        function (n) {
            // get layout works for both byId & byName
            layoutArgs.push(n);
            that.layoutDBAction.get.apply(that.layoutDBAction, layoutArgs)
        }
    ], function (err, result) {
        if (!err) {
            var theme = result[0], layout = result[1];
...
PageRenderer.prototype._eachNS = function (ns, next) {
    ...
    if (ns) {
        ...
        async.waterfall([
            function (n) {
                that.pluginInstanceDBAction.get("findByPluginNamespaceAndPageId", [ns, page.pageId], n)
            },
            function (pIModel, n) {
...
PageRenderer.prototype.getTopMenu = function (next) {
    var that = this;
    var query = that.pageInstanceDBAction.getQuery().where('friendlyURL')
        .select('friendlyURL localizedName')
        .where("parentPageId", 0)
        .where("isHidden", false)
        .sort("pageId");
    that.pageInstanceDBAction.authorizedGetByQuery(query, next);
};
...
PageRenderer.prototype.render = function (next) {
    ...
    async.waterfall([
        function (n) {
            that._init(["findByLayoutId", page.layoutId], n);
        },
        function (n) {
            that._render(n);
        }
...
PageRenderer.prototype.renderMaximized = function (next) {
    ...
    async.waterfall([
        ...
        function (n) {
            that._init(["findByName", LAYOUT_ONE_COLUMN_NAME], n);
        },
        function (n) {
            that._render(n);
        }
