...
module.exports = function (app) {
    ...
    var PageService = app.getService(PAGE_SCHEMA),
        PageAuthService = PageService.Auth;
...
//        Debug._l("url >  " + url);
    ...
    app.post(AppURL + '/updatePageData', function (req, res) {
        ...
        PageAuthService.updateByReq(req, PAGE_PERMISSION_SCHEMA_KEY, {
            pageId: pageId,
            data: pageData
        }, function (err, result) {
            if (err) {
    ...
    app.get(AppURL + '/getPagePlugins', function (req, res) {
        ...
        if (pageId) {
            PageAuthService.findByIdAndReq(req, PAGE_PERMISSION_SCHEMA_KEY, pageId, function (err, page) {
            ...
            PageAuthService.findByIdAndReq(req, PAGE_PERMISSION_SCHEMA_KEY, pageId, function (err, page) {
                if (page) {
                    ...
                    PluginInstanceHandler.addInstance(req, ns, pageId, plugin.name, function (err, result) {
                        if (result) {
                            PageAuthService.updateByReq(req, PAGE_PERMISSION_SCHEMA_KEY, obj, function (err, result) {
                                if (result) {
...
//                            delete value[idx];
                            ...
                            PluginInstanceHandler.deleteInstance(req, ns, pageId, function (err, result) {
                                if (result) {
                                    PageAuthService.update(obj, function (err, result) {
                                        if (result) {
                                            res.send({success: true});
                                        }
                                        else {
