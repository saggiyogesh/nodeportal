...
var checkPagePermission = require("./permissions/PagePermission");
...
function getRoute(params) {
    ...
    return (params.isAppRoute ? AppURL + "/" : "/:locale?/:page/") + params.pluginId + (params.many ? "/:iId" : "") + params.route;
}
...
function Route(method, route, fn) {
    ...
    return function (app) {
        var args = [method, route, checkPagePermission, fn];
