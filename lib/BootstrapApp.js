...
function populateAppProperties(props) {
    AppProperties.set("DB_TYPE", props["db.type"]);
    ...
    AppProperties.set("DB_DEBUG", props["db.debug"]);
...
//        props['db.port'] + "/" + props['db.name']);
    ...
    AppProperties.set("HTTP_SERVER_PORT", props["http.server.port"]);
    AppProperties.set("HTTPS_SERVER_PORT", props["https.server.port"]);
    AppProperties.set("HTTPS_CERTIFICATE_FILE_PATH", props["https.certificate.file.path"]);
    AppProperties.set("HTTPS_KEY_FILE_PATH", props["https.key.file.path"]);
...
module.exports = function (app) {
    ...
    fs.existsSync(overrideFilePath) && readParseProperties(overrideFilePath);
