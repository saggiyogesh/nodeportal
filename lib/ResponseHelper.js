...
exports.handleXHRErrorResponse = function handleXHRErrorResponse(req, res) {
    //check for xhr request err
    //this is handled in different way
    var ret = req.attrs.value;
    var errHeader = ret[1];
    if (req.xhr && utils.containsArray([500, 401, 404], errHeader || res.statusCode)) {
        res.status(errHeader).send(ret[0]);
    }
};
