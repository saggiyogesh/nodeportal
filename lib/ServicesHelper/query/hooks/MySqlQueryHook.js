...
MySqlQueryHook.prototype.authorizedFind = function (roles, modelName, filter, next) {
    ...
    var that = this, connector = that.getConnector();
    var q = [];
    q.push(that.buildQuery(modelName, {}));
    if (filter.where) {
    ...
    q.push(that.buildPermissionQuery(modelName, roles, that.getViewBitValue()));
    filter.order && q.push(that.buildOrderBy(modelName, filter.order));
    filter.limit && q.push(that.buildLimit(filter.limit, filter.skip || filter.offset || 0));

    that.executeQuery(q.join(' '), function (err, data) {
        if (err) {
            return next(err, []);
        }
        data && !err && (data = that.toDataModel(modelName, data, filter));
        next(err, data);
    });
};
...
MySqlQueryHook.prototype.buildLikeFilter = function (where, column, keyword, isNotLike) {
