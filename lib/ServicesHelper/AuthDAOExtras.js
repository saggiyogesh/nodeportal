...
function AuthDAOExtras(ServiceAuth) {
    ...
    ServiceAuth.populateModelAndSave = function populateModelAndSaveAuth(params, otherValues, keyMapObj, pv, next) {
        pv.hasPermissionWithoutModelId(PermissionActions.ADD, function (err, perm) {
            if (perm && perm.isAuthorized) {
                baseService.populateModelAndSave(params, otherValues, keyMapObj, next);
            }
        });
    };

    ServiceAuth.populateModelAndUpdate = function populateModelAndUpdateAuth(params, otherValues, keyMapObj, pv, next) {
        pv.hasPermission(PermissionActions.UPDATE, params[baseService.getIdName()], function (err, perm) {
            if (perm && perm.isAuthorized) {
                baseService.populateModelAndUpdate(params, otherValues, keyMapObj, next);
            }
        });
    };
}