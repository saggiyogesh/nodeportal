...
function AuthDAOExtras(ServiceAuth) {
     ...
     */
    ServiceAuth.removeByQuery = function removeByQueryAuth(where, roles, next) {
     ...
     * @param next {Function} callback. Parameters : err & boolean flag in case of no err
     */
    ServiceAuth.updateByQuery = function updateByQueryAuth(data, where, roles, next) {
    ...
    /**
     * Helper to save model data by req object.
     * @param req {Object} Request object
     * @param permissionSchemaKey {String}
     * @param modelData {Object} model data to be saved.
     * @param next {Function} callback. Parameters : err & boolean flag in case of no err
     */
    ServiceAuth.saveByReq = function saveByReq(req, permissionSchemaKey, modelData, next) {
        this.save(modelData, createPermissionValidator(req, permissionSchemaKey), next);
    };
    ...
    ServiceAuth.populateModelAndSave = function populateModelAndSaveAuth(formData, otherValues, keyMapObj, pv, next) {
        ...
        pv.hasPermissionWithoutModelId(PermissionActions.ADD.name, function (err, perm) {
            if (perm && perm.isAuthorized) {
                var modelValues = baseService.populateModel(formData, otherValues, keyMapObj);
                that.save(modelValues, pv, next);
            }
        });
    };
    ...
    ServiceAuth.populateModelAndUpdate = function populateModelAndUpdateAuth(formData, otherValues, keyMapObj, pv, next) {
        ...
        pv.hasPermission(PermissionActions.UPDATE.name, formData[baseService.getIdName()], function (err, perm) {
            if (perm && perm.isAuthorized) {
                var modelValues = baseService.populateModel(formData, otherValues, keyMapObj);
                var pkValue = modelValues[baseService.getIdName()];
                that.updateById(pkValue, modelValues, pv, next);
            }
        });
    };

    ServiceAuth.paginate = function (requestQuery, colNames, roles, next) {
        var that = this, paginationParams = baseService.getPaginationParams(requestQuery, colNames);
        async.parallel({
            total: function (n) {
                that.count(paginationParams.where, roles, n);
            },
            data: function (n) {
                that.find(paginationParams, roles, n);
            }}, function (err, result) {
            var ret = {};
            if (!err) {
                ret = {
