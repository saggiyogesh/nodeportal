...
function DAOExtras(Service) {
     ...
     * @returns Object
     */
    Service.populateModel = function populateModel(params, otherValues, keyMapObj) {
        if (!params) {
    ...
    Service.populateModelAndUpdate = function (formData, otherValues, keyMapObj, next) {
        var modelValues = this.populateModel(formData, otherValues, keyMapObj);
    ...
    Service.getPaginationParams = function (requestQuery, colNames) {
        var sortCol = requestQuery["iSortCol_0"], sortDir = requestQuery["sSortDir_0"],
            start = requestQuery["iDisplayStart"], length = requestQuery["iDisplayLength"],
            searchKeyword = requestQuery["sSearch"], where;

        if (searchKeyword) {
            where = {};
            this.getDataSource().queryHook.buildLikeFilter(where, "localizedTitle", searchKeyword);
        }
        return {
            where: where,
            order: colNames[sortCol] + ' ' + (sortDir === 'asc' ? 'ASC' : 'DESC'),
            limit: length,
            skip: start
        };
    };

    Service.paginate = function (requestQuery, colNames, next) {
        var that = this, paginationParams = that.getPaginationParams(requestQuery, colNames);
        async.parallel({
            total: function (n) {
                that.count(paginationParams.where, n);
            },
            data: function (n) {
                that.find(paginationParams, n);
            }}, function (err, result) {
            var ret = {};
            if (!err) {
                ret = {
