...
function generateHooks(confFile, tmplOptions) {
    //currently before hooks is created.
    //after hooks should not be used, as if used then model events will not work

    var allowedHooks = ['validate', 'save', 'create', 'update', 'destroy'],
        hooks = confFile.hooks;

    tmplOptions.hooks = [];
    if (hooks) {
        allowedHooks.forEach(function (type) {
            var handlerFn = hooks[type];
            if (handlerFn) {
                var string = "before" + utils.capitalizeFirstLetter(type) + " = " + handlerFn.toString() + ";";
                tmplOptions.hooks.push(string);
            }
        });
    }
}
...
//    utils.isWin() && tmplOptions.path.replace(/\\/g, "/");
    ...
    generateHooks(confFile, tmplOptions);
